  ...
  return (
    <div className="flex-1 min-h-screen" style={{ background: "#080b14" }}>
      ...
      <div className="px-4 sm:px-6 py-4 sm:py-6 border-b border-white/5">
        <h1 className="text-white font-bold text-xl sm:text-2xl">Reportes</h1>
        <p className="text-slate-500 text-sm mt-0.5">Análisis y comparativas de tus finanzas</p>

        {/* Mode tabs */}
        <div className="flex gap-1 mt-5 bg-white/4 p-1 rounded-xl w-fit border border-white/5">
          {([["evolucion","Evolución temporal"],["comparar","Comparar períodos"]] as const).map(([v,l]) => (
        ...
        {mode === "evolucion" && (
          <>
            ...
            <div className="flex gap-2 flex-wrap">
              {([["6m","6 meses"],["12m","12 meses"],["year","Este año"]] as [EvolPeriod,string][]).map(([v,l]) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                { label:"Total ingresos", value:totalIncome, color:"#10b981", icon:TrendingUp },
                { label:"Total gastos",   value:totalExpense, color:"#ef4444", icon:TrendingDown },
                { label:"Balance neto",  value:totalIncome-totalExpense, color:(totalIncome-totalExpense)>=0?"#10b981":"#ef4444", icon:DollarSign },
