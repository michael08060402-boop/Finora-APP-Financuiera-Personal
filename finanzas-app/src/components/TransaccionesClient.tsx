    ...
    <div className="flex-1 flex flex-col min-h-screen" style={{ background: "#080b14" }}>
      ...
      <div className="px-4 sm:px-6 py-4 sm:py-6 border-b border-white/5">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4 mt-4">
          {[
            { label: "Balance", value: balance, color: balance >= 0 ? "text-emerald-400" : "text-red-400", border: "border-white/5" },
            { label: "Ingresos", value: totalIncome, color: "text-emerald-400", border: "border-emerald-500/20" },
            { label: "Gastos", value: totalExpense, color: "text-red-400", border: "border-red-500/20" },
          ].map((c) => (
            <div key={c.label} className={`rounded-xl p-4 border ${c.border}`} style={{ background: "rgba(255,255,255,0.03)" }}>
              <p className="text-slate-500 text-xs uppercase tracking-wider">{c.label}</p>
              <p className={`font-bold text-lg mt-1 ${c.color}`}>{fmt(c.value)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 sm:px-6 py-3 flex flex-wrap gap-2 items-center border-b border-white/5">
        ...
        <div className="flex items-center gap-1 bg-white/5 rounded-xl p-1">
          {[["all", "Todos"], ["income", "Ingresos"], ["expense", "Gastos"]].map(([val, lbl]) => (
            <button
              key={val}
